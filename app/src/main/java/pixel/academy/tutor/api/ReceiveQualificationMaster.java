...
public class ReceiveQualificationMaster
{
	private Context context;
	private SQLiteHelper helper;

	public ReceiveQualificationMaster(Context context)
	{
	...
	public void execute()
	{
		...
		final String URL = context.getResources().getString(R.string.pixelServerBaseUrl)
				+ context.getResources().getString(R.string.pixelServerQualificationMasterUrl);
				...
				try
				{
					...
					if(arr.length() > 0)
					{
						for (int i = 0; i < arr.length(); i++)
						{
							...
							String qualification = jsonObj.getString(KEY_QUALIFICATION);
							String timestamp = jsonObj.getString(KEY_TIMESTAMP);

							helper.master_insert(new Qualification(id, qualification, timestamp));
						}
					}
				}
