...
public class ReceiveBoardMaster
{
	private Context context;
	private SQLiteHelper helper;

	public ReceiveBoardMaster(Context context)
	{
	...
	public void execute()
	{
		...
		final String URL = context.getResources().getString(R.string.pixelServerBaseUrl)
				+ context.getResources().getString(R.string.pixelServerBoardMasterUrl);
				...
				try
				{
					...
					if(arr.length() > 0)
					{
						for (int i = 0; i < arr.length(); i++)
						{
							...
							String board = jsonObj.getString(KEY_BOARD);
							String timestamp = jsonObj.getString(KEY_TIMESTAMP);

							helper.master_insert(new Board(id, board, timestamp));
						}
					}
				}
