...
public class ReceiveSubjectMaster
{
	private Context context;
	private SQLiteHelper helper;

	public ReceiveSubjectMaster(Context context)
	{
	...
	public void execute()
	{
		...
		final String URL = context.getResources().getString(R.string.pixelServerBaseUrl)
				+ context.getResources().getString(R.string.pixelServerSubjectMasterUrl);
				...
				try
				{
					...
					if(arr.length() > 0)
					{
						for (int i = 0; i < arr.length(); i++)
						{
							...
							String subject = jsonObj.getString(KEY_SUBJECT);
							String timestamp = jsonObj.getString(KEY_TIMESTAMP);

							helper.master_insert(new Subject(id, subject, timestamp));
						}
					}
				}
