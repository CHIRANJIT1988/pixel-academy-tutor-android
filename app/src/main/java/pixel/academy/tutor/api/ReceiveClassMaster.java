...
public class ReceiveClassMaster
{
	private Context context;
	private SQLiteHelper helper;

	public ReceiveClassMaster(Context context)
	{
	...
	public void execute()
	{
		...
		final String URL = context.getResources().getString(R.string.pixelServerBaseUrl)
				+ context.getResources().getString(R.string.pixelServerClassMasterUrl);
				...
				try
				{
					...
					if(arr.length() > 0)
					{
						for (int i = 0; i < arr.length(); i++)
						{
							...
							int _id = jsonObj.getInt(KEY_ID);
							String _class = jsonObj.getString(KEY_CLASS);
							String _timestamp = jsonObj.getString(KEY_TIMESTAMP);

							helper.master_insert(new Standard(_id, _class, _timestamp));
						}
					}
				}
